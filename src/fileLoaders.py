...
class GithubLoader(BaseLoader):
    ...
    def __init__(self, user, repo, subdir=None, sha=None, prov=None):
        ...
        try:
            ...
            raise Exception(
                "BadCredentials: have you set up github_access_token on config.ini ?"
            )
        except Exception:
    ...
    def _fetchFilesFromPath(self, path):
        ...
        path = path.strip("/")
        contents = self.gh_repo.get_contents(path, ref=self.sha)
        files = []
...
class GitlabLoader(BaseLoader):
    def __init__(self, user, repo, subdir=None, sha=None, prov=None, branch=None):
        ...
        self.prov = prov
        gl = gitlab.Gitlab(
            url=static.GITLAB_URL, private_token=static.GITLAB_ACCESS_TOKEN
        )
        try:
            ...
            raise Exception(
                "GitlabAuthenticationError: have you set up gitlab_access_token on config.ini ?"
            )
        except Exception:
            raise Exception("Repo not found: " + user + "/" + repo)
    ...
    def _fetchFilesFromPath(self, path):
        ...
        gitlab_files = self.gl_repo.repository_tree(
            path=path, ref=self.branch, all=True
        )
        files = []
    ...
    def getRawRepoUri(self):
        ...
        return path.join(
            static.GITLAB_URL, self.user, self.repo, "-", "raw", self.branch
        )
...
class LocalLoader(BaseLoader):
    ...
    def fetchFiles(self):
        ...
        for f in files:
            ...
            filesDef.append({"download_url": relative, "name": relative})
        return filesDef
...
class URLLoader(BaseLoader):
    ...
    def __init__(self, spec_url):
        ...
        if resp.status_code == 200:
            ...
            for query in self.spec["queries"]:
                ...
                item = {"name": queryName, "download_url": queryUrl}
    ...
    def fetchFiles(self):
        ...
        files = [v for k, v in self.spec["files"].items()]
        return files
    ...
    def _getText(self, itemName):
        ...
        if itemName in self.spec["files"]:
            ...
            )  # Join with base URL if relative URL
            resp = requests.get(itemUrl, headers=headers)
            if resp.status_code == 200:
                return resp.text
            else:
                raise Exception(
                    "HTTP status {} encountered while loading {}".format(
                        resp.status_code, itemUrl
                    )
                )
        else:
            return None
